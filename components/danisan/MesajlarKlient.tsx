...
export default function MesajlarKlient({ konusmalar }: Props) {
  ...
  const filtreliListe = useMemo(() => {
    ...
    if (aramaMetni.trim()) {
      ...
      liste = liste.filter((k) => k.musteriIsim.toLowerCase().includes(q));
    }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[11px] text-[#4A4D4A] mt-0.5">
            Danışanlarınızla konuşmalar
          </p>
        </div>
        {toplamOkunmamis > 0 && (
          <div className="bg-[#A6DE9B] text-[#325343] text-[11px] font-bold px-2.5 py-1">
              ...
              >
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 border-b border-[#D4E4D8] dark:border-[#333] hover:bg-[#F5F7F5] dark:hover:bg-[#111] transition-colors group ${
                    k.okunmamisSayi > 0 ? "bg-white dark:bg-[#1a1a1a]" : ""
                  }`}
                >
                  {/* Avatar */}
                  <div className="w-9 h-9 shrink-0 bg-[#D4E4D8] dark:bg-[#333] flex items-center justify-center text-[13px] font-bold text-[#252625] dark:text-white relative">
                    {k.musteriIsim.charAt(0).toUpperCase()}
                    {k.okunmamisSayi > 0 && (
                      <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#A6DE9B] text-[#325343] text-[8px] font-bold flex items-center justify-center">
