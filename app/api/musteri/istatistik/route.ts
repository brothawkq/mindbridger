...
export async function GET() {
  ...
  const [{ data: randevuDanisan }, { data: blogDanisanlar }] =
    await Promise.all([
      ...
      blogDanisanIds.length > 0
        ...
        : Promise.resolve({ data: [] as { id: string; title: string | null; profiles: { first_name: string | null; last_name: string | null } | null }[] }),
    ]);
  ...
  const tamamlananGorev = Object.values(haftaGorevler).filter(Boolean).length;
